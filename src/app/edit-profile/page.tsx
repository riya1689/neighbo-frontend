...
export default function EditProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem("user", JSON.stringify({ ...storedUser, displayName: data.user.displayName, bio: data.user.bio }));
      } else {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewPassword("");
        setConfirmPassword("");
        fetchProfile(); // Refresh to get new passwordLastUpdatedAt
      } else {
  ...
  const getRemainingDays = () => {
    ...
    const lastUpdate = new Date(user.nameLastUpdatedAt);
    const diff = new Date().getTime() - lastUpdate.getTime();
    return Math.ceil(28 - diff / (24 * 60 * 60 * 1000));
  };
            ...
            <div className="space-y-8">
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      />
                      <UserIcon size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 ${isNameLocked() ? "text-slate-300" : "text-primary/60"}`} />
                    </div>
                    {isNameLocked() && (
                      <p className="text-[10px] text-amber-500 font-bold flex items-center gap-1 mt-1 bg-amber-50 p-2 rounded-lg">
                        <Clock size={12} />
                        You can update your name in {getRemainingDays()} days.
                      </p>
                    )}
                  </div>
                  ...
                  >
                    {isSaving ? "Saving..." : <><CheckCircle2 size={18} /> Update Profile</>}
                  </button>
                </form>
              </div>
              ...
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100">
                <form onSubmit={handleUpdatePassword} className="space-y-6">
                  <div className="flex items-center justify-between border-b border-slate-50 pb-4">
                    ...
                    {user?.passwordLastUpdatedAt && (
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1">
                        <Clock size={10} />
                        Updated {formatDistanceToNow(new Date(user.passwordLastUpdatedAt))} ago
                      </span>
                    )}
                  </div>
                  ...
                  >
                    {isSaving ? "Updating..." : <><Lock size={18} /> Change Password</>}
                  </button>
                </form>
              </div>

              {/* Account Status / Role Section */}
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-4">
                 <div className="space-y-1">
                   <h2 className="text-sm font-bold text-slate-800">Account Standing</h2>
                   <p className="text-xs text-slate-400 font-medium">Your current roles and privileges within Neighbo.</p>
                 </div>
                 <div className="flex items-center gap-3">
                   <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 ${
                     user?.role === 'ADMIN' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-primary/5 text-primary border-primary/10'
                   }`}>
                     <Shield size={12} />
                     {user?.role}
                   </div>
                   {(user?.subscriptions?.length > 0 || user?.isPremium) && (
